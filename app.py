...
app.title= "Desempenho por localidade"
...
db = create_engine('postgresql://postgres:123456@localhost/postgres')
...
def getEstadoCities(estado, variavel='mediatodos', varLocal="SG_UF_ESC"):
    ...
    data = pd.read_sql(f""" select rank() over(ORDER BY  avg("{variavel}")  DESC) Ranking,  "{municipio}",  avg("{variavel}") as Media  from enem2018  where  "{varLocal}"='{estado}' group by "{municipio}" order by Media desc;""", con=db)
